...
function endGame(who) {
  try {
    ...
    // Added a prompt to ask if the user wants to play again
    setTimeout(function () {
      try {
        let playAgain = confirm("Would you like to play again?");
        if (playAgain) {
          restartGame(); // If the user wants to continue, restart the game
        } else {
          alert("Thank you for playing! Game developed by Nathan Forrest."); // Thanking the user and credit
        }
      } catch (error) {
        // Handling any errors during the prompt
        console.error("An error occurred during the play again prompt:", error);
        alert(
          "Oops, something went wrong while asking to play again. Please try reloading the game."
        );
      }
    }, 800); // Added a delay before showing the prompt
  } catch (error) {
    // Catching any general errors during the ending game process
    console.error("An error occurred in the endGame function:", error);
    alert("Oops, something went wrong. The game will restart.");
    restartGame(); // Restart the game if there's an error
  }
}
...
function updateTimerDisplay() {
    ...
    if (timerDisplay) {
        timerDisplay.innerText = "Time: " + timeElapsed + " seconds";
    }
}

// Start the timer
function startTimer() {
    // Start counting only if it's not already running
    if (!timer) {
...
function restartGame(ask) {
    ...
    stopTimer(); // Stop the timer when the game is restarted
    gameOver = false;
    moves = 0;
    winner = 0;
    whoseTurn = x;
    ...
    // Start the timer again after stopping it
    startTimer();
